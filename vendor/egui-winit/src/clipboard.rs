...
fn init_smithay_clipboard(
    ...
    // Stock egui-winit behaviour, deliberately restored. An earlier Rune fork returned None here to
    // force get()/set() down arboard's wlr-data-control backend; that stopped working once the
    // compositor gated zwlr_data_control_manager_v1 to rclip alone (clipboard-sniffing defence), and
    // with the core path also switched off, egui apps were left with no clipboard at all. The
    // focus-gated wl_data_device path below is the one every unprivileged client is meant to use,
    // and Rune serves it (SeatHandler::focus_changed → set_data_device_focus).
    if let Some(RawDisplayHandle::Wayland(display)) = raw_display_handle {
        ...
        #[cfg(feature = "wayland")]
        log::debug!("Cannot init smithay clipboard without a display handle");
        None
    }
}
