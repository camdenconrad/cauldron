//! Content-Length framing — the only wire format stdio LSP servers speak.
//!
//! The reader is deliberately garbage-tolerant: shell-wrapped servers print banners and npm
//! chatter before their first real header, so any line that isn't a header we recognize is
...
/// Read one framed message off the server's stdout.
///
    ...
    let len = loop {
        ...
        if r.read_line(header_buf)? == 0 {
            return match content_length {
                ...
                Some(_) => {
                    header_buf.clear();
                    Err(io::Error::new(io::ErrorKind::UnexpectedEof, "eof inside message headers"))
                }
            };
        }
...
mod tests {
    ...
    fn write_then_read_round_trips() {
        let v = json!({"jsonrpc": "2.0", "id": 1, "method": "initialize",
                       "params": {"rootUri": "file:///tmp/x", "text": "line1\r\nline2"}});
    ...
    fn junk_lines_before_the_first_header_are_skipped() {
        // Shell-wrapped servers print banners (and even blank lines) before the first frame.
    ...
    fn header_name_matches_case_insensitively() {
        assert_eq!(read(b"content-length: 2\r\n\r\n{}").unwrap().unwrap(), json!({}));
        assert_eq!(read(b"CONTENT-LENGTH: 2\r\n\r\n{}").unwrap().unwrap(), json!({}));
    }
    ...
    fn eof_at_message_boundary_is_clean() {
        ...
        // Trailing chatter from a dying server, with no Content-Length, is still a boundary.
        assert!(read(b"server exited unexpectedly\n").unwrap().is_none());
    }
    ...
    fn two_messages_back_to_back() {
        let mut wire = Vec::new();
        write_message(&mut wire, &json!({"id": 1, "result": "first"})).unwrap();
        write_message(&mut wire, &json!({"id": 2, "result": "second"})).unwrap();
        // One cursor, shared buffers — exactly how the reader thread consumes the stream.
        let mut cur = Cursor::new(wire.as_slice());
        let mut header = String::new();
        let mut body = Vec::new();
        let a = read_message(&mut cur, &mut header, &mut body).unwrap().unwrap();
        let b = read_message(&mut cur, &mut header, &mut body).unwrap().unwrap();
        assert_eq!(a["result"], "first");
        assert_eq!(b["result"], "second");
        assert!(read_message(&mut cur, &mut header, &mut body).unwrap().is_none());
    }
}
